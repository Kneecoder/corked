...
Generated: 2026-07-12T15:49:08.505Z
...
```json
{
  ...
  "digestibility": {
    "state": "cellar_ready",
    "missing": [],
    "reason": "Cellar-ready. The role is one-man trade businesses and the stated problem is forgetting to chase unpaid invoices until the money is already a problem. Both are present and the Spark enters cleanly.",
    "can_seal": true,
  ...
  "m1_setup": {
    "question": "Name the specific one-man trade business owner this Spark starts from and the moment they realised unpaid invoices had become a money problem.",
    "hint": "A good answer names a real person, their trade, and a concrete moment when the unchased invoice pile caught up with them.",
    "person_label": "Tradesperson",
    "person_placeholder": "e.g. Danny, sole-trader electrician",
    "when_label": "When the money became a problem",
    "when_placeholder": "e.g. three weeks after finishing a job, when rent was due",
    "trying_label": "What they were trying to do",
    "trying_placeholder": "e.g. cover the month's expenses while waiting on four outstanding invoices",
    "happened_label": "What actually happened",
    "happened_placeholder": "e.g. he checked his account and realised two invoices from six weeks ago had never been followed up and were still unpaid"
  },
  "person_in_spark": {
    "grade": "none",
    "named_person": null,
...
```json
{
  ...
  "observation": {
    "surface_text": "A named individual is present. The relationship is specific and real: a one-woman landscaping business owner known personally to the respondent.",
...
```json
{
  ...
  "gap_in_play": false,
  "observation": {
    "surface_text": "The spark names the problem directly. Noor runs a one-woman landscaping business, which places her inside the implied person.",
    "problem_source": "explicit"
  },
  "question": "Think of the last time Noor realised a invoice had gone unpaid too long, what did she actually do when she noticed it?",
  "hint": "A good answer names a specific job, how many weeks the invoice had been sitting, and the action Noor took when she finally caught it.",
  "server_checks": {
    "schema_valid": true,
    "gap_in_play": false,
    "needs_confirmation": false,
...
```json
{
  ...
  "observation": {
    "surface_text": "The Tell contains an inferred pattern rather than a specific observable action a camera would capture. No single past instance is named in the answer; the moment is entirely unanchored.",
    "anchor_span": "usually she just puts off looking at the numbers"
  },
  "substitution_detected": false,
  "swirl": {
    "kind": "reality",
    "question": "Is there one specific time Noor went quiet on an overdue invoice and something observable happened as a result?"
  },
  "server_checks": {
    "schema_valid": true,
...
```json
{
  ...
  "observation": {
    "surface_text": "The Echo bar finds one identifiable separate person, Jordan, not the grape and not the founder, with a concrete workaround: paying a freelancer to handle it. The Vintage bar finds a recurring pattern rather than a single anchored instance.",
    "anchor_span": "my colleague Jordan mentioned he hits this same thing and just pays a freelancer to sort it out whenever it comes up"
  },
  "substitution_detected": false,
  "server_checks": {
...
```json
{
  ...
  "observation": {
    "surface_text": "The answer names Priya and attributes a concrete workaround, but the role and context of this person are not described with enough specificity to confirm she is an identifiable individual rather than a constructed stand-in. No specific past instance is anchored; the workaround is stated as a standing practice.",
    "anchor_span": "A friend named Priya, who works in a completely different setup, said she keeps a manual backup log for it"
  },
  "substitution_detected": false,
  "swirl": {
    "kind": "reality",
    "question": "Is Priya a specific person the answer's speaker has spoken with directly about this, or a composite of people who handle it this way?"
  },
  "server_checks": {
    "schema_valid": true,
    "visible_style_violations_cleaned": [],
...
```json
{
  ...
  "observation": {
    "surface_text": "The answer names an existing solution: a plain spreadsheet template. The failure point given is that it does not catch the problem until too late, which is a timing complaint without a precise mechanism, it does not state what the spreadsheet fails to do specifically.",
    "anchor_span": "it does not catch the problem until it is already too late"
  },
  "substitution_detected": false,
  "swirl": {
    "kind": "reality",
    "question": "When Noor opens that spreadsheet, what is the specific thing it cannot do that lets an overdue invoice slip past, does it lack a due-date column, send no reminder, or something else?"
  },
  "server_checks": {
    "schema_valid": true,
    "visible_style_violations_cleaned": [
...
```json
{
  ...
  "observation": {
    "surface_text": "A direct quote from a named individual in a real, dated exchange is present, meeting the Words bar. The words carry a stated sentiment about the problem but no filmable action or third-party reference.",
    "anchor_span": "\"I just want this part to be boring,\" Noor said, when we talked about it last week."
  },
  "substitution_detected": false,
  "server_checks": {
...
```json
{
  ...
  "observation": {
    "surface_text": "The answer names a specific individual with a bounded structural arrangement: retainer-only billing. The reason for immunity is stated and specific: payment accrues monthly regardless of whether work occurs, eliminating the unpaid-invoice exposure.",
    "anchor_span": "only ever takes retainer clients, so this never happens to him. He is paid whether the job happens or not."
  },
  "substitution_detected": false,
  "server_checks": {
